...
//		}
	...
	{
		HashMap<String, Variable> allVars = context.allParentsLocalVarsHash(new HashMap<String, Variable>());
		if (allVars.containsKey(tokens.get(cnt).text))
		{
			Variable v = allVars.get(tokens.get(cnt).text);
...
//call / execution before method call
	...
	public String recursivelyFindCollections(HashMap<String, Trigger> events, Trigger event, String list, ArrayList<Trigger> array)
	{
		if (EventCollection.reverse.containsKey(event.name.text))
			for (String s : EventCollection.reverse.get(event.name.text).keySet())
			{
				...
				String temp2 = recursivelyFindCollections(events, events.get(s), list, array);
				if (!list.contains(", "+  events.get(s).getId() + "/*"))
				{
					list = temp + temp2;
					array.add(events.get(s));
				}
				else
					list = temp2;
			}
...
//		allWhere.addAll(recursiveWhereClauses(events, this, new ArrayList<Token>()));
		...
		throws ParseException
	{
		...
		{
			sb.append("Clock _c, long millis) : (call(* Clock.event(long))" +
					" && args(millis) && target(_c) && (if (millis % "+clockAmount+"l==0))"+
					" && !cflow(adviceexecution())");
		}
		else if (type == EventType.channel)
		{
			String className = "_cls_"+g.name;
			
			Global temp = g;
			while (!temp.localVariables.containsKey(this.channelName.text))
				temp = temp.parent;
			className += temp.id;
			
			sb.append("Channel _c) : (call(* Channel.receive(..)) && target(_c) && (if (_c.equals("+className+"."+ this.channelName +")))");
...
//				sb.append("*");
		...
		ArrayList<Trigger> collectionList = new ArrayList<Trigger>();
		//handle collection events
		s.append(recursivelyFindCollections(events, this, "",collectionList));

		
		for (Trigger t : collectionList)
		...
		//invoking the global running clock
		if (this.variables.containsKey("timestamp"))
		...
		//method body
			...
			if (!this.type.equals(EventType.clock) && !this.type.equals(EventType.clockCycle))
			{
				...
				sb.append("\r\n_cls_"+g.name+g.id+" _cls_inst = _cls_"+g.name+g.id+"._get_cls_"+g.name+g.id+"_inst(false");
				for (Variable v:allParentVariables)
					sb.append("," + v.name);
				//sb.deleteCharAt(sb.length()-1);
				sb.append(");");
...
//					else
		...
		//	if (g.parent != null)//note that now global has been persisted as well!!
			{
			if (!this.type.equals(EventType.clock))
