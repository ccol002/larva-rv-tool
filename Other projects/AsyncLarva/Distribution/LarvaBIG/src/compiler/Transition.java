...
public class Transition {
	...
	{	
		...
		for (Arrow a:arrows)
		{
			...
			sb.append("){");
			sb.append("\r\n		"+Global.redirectOutput(Global.handleVariableReplacement(a.action,g), "[" + l.name + "]"));
			sb.append("\r\n		_state_id_"+l.name+" = "+a.destination.id+";//moving to state "+a.destination);
			
			if (a.destination.type == State.Type.ACCEPTING)
			{
				//MB - needs to be changed
				sb.append("\r\n		no_automata--;");
				sb.append("\r\n		if (no_automata == 0)");
				sb.append("\r\n		{ _cls_" + g.name + g.id + ".livingObjects--; }");
			}
			
			if (a.destination.code != null)
				sb.append("\r\n" +Global.redirectOutput(Global.handleVariableReplacement(a.destination.code,g),"[" + l.name + "]"));
