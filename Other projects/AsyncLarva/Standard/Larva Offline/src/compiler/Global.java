...
//				}
	...
	public void createFileForEach()throws ParseException, Exception
	{
		...
		if (this.parent == null)
		{
			...
			for (Variable v:localVariables.values())
				if (v.getVariableType().equals("Clock"))
					cl.append("\r\nroot." + v.getVariableName() + ".reset(RunningClock.now);");
			
			if (initializationCode != null && initializationCode.size() > 0)
			{
				cl.append("root.specialInitialization(" + initializationCode.get(0) + "());");
			}
		...
		else
		{
			...
			{
				cl.append("\r\nfor (Object o : al)" +
						"\r\n  _get_cls_"+name+id+"_inst(null, "
					+ "(" + variables.get(0).getVariableType()+")o);");
			}
			else
			{
			cl.append("\r\nfor (Object o : al)" +
					"\r\n  _get_cls_"+name+id+"_inst(" 
				+ initializationCode.get(0).text 
				+ "((" + variables.get(0).getVariableType()+")o),"
				+ " (" + variables.get(0).getVariableType()+")o);");
			}
			cl.append("\r\n}");
...
//		cl.append("\r\n\r\npublic void ForeachInitialize(ArrayList a)" +
//				"\r\n{" +
//				"\r\ntry {" +
//				"\r\n  for (Object o : a){" +
//				"\r\n    //generate a monitor for each object" +
//				"//if this context has a nested foreach then its foreachinitialize must also be called" +
//				
//				"\r\n}" +
//				"\r\n} catch(Exception ex)" +
//				"\r\n{ex.printStackTrace();}");
		...
		if (parent != null)
		{
			ArrayList<Variable> parVars = parent.allParentsVars(new ArrayList<Variable>());
			
			if (initializationCode == null || initializationCode.size() ==0)
			{
				cl.append("\r\nparent = _cls_"+name+parent.id+"._get_cls_"+name+parent.id+"_inst(null, ");
...
//						cl.append("\r\n" + v.getVariableName() + ".registerCycle(" + l + ");");
		...
		cl.append("\r\n\r\npublic static _cls_"+name+id+" _get_cls_"+name+id+"_inst(HashMap<String,Object> inits");
		for (Variable v: allParentsVars(new ArrayList<Variable>()))
			cl.append("," + v.type + " " + v.name);
		cl.delete(cl.length()-1, cl.length());//remove last comma
