...
public class Property extends Compiler {
	...
	{
		sb.append("\r\npublic int _state_id_"+name+" = "+states.starting.get(0).id+";");
		//translate logics
		...
		if (Compiler.verbose){
			sb.append("\r\n_cls_"+root.name+root.id+"._pw.println(\"[" + name + "]MOVED ON METHODCALL: \"+ _info +\" TO STATE::> \" + _string_"+name+"(_state_id_"+name+", 1));");
			sb.append("\r\n_cls_"+root.name+root.id+"._pw.flush();");
		}
		sb.append("\r\n}");
	...
	public void outputDiagram(String outputDir) 
	{
		...
		for (Transition t:transitions.transitions.values())
		{
			...
			for (Arrow a:t.arrows)
			{
				...
				if (show_transition_text)
				{
					...
					sb.append(" [label = \"");
					if (show_transition_priority)
						sb.append("(" + cnt + ") ");
					sb.append(a.trigger);
					String condition = Tokenizer.show2(a.condition);
					if (condition.length() > 25)
						sb.append("\\\\" + condition.substring(0, 25) + "..."); //+ "\\"
				...
				else if (show_transition_priority)
				{
					sb.append(" [label = \"("+cnt+")\"];");
				}
			}			
		...
		try{
			PrintWriter pw1 = new PrintWriter(Compiler.outputDir+"\\diags\\_diag_"+name+".txt");
			pw1.write(sb.toString());
			pw1.close();
			Runtime.getRuntime().exec("\""+Compiler.graphvizDir+"bin\\dot.exe\"  -Tgif -o\""+outputDir
					+"_logic_"+name+"_diag.gif\" -Kdot \""+Compiler.outputDir+"\\diags\\_diag_"+name+".txt\"");
