...
public class Repeat extends F{
	
	F f;
	
	public int parse(ArrayList<Token> tokens, int cnt) throws ParseException {
		
		if (tokens.get(cnt).is("repeat"))
			cnt++;
		else throw new ParseException("repeat expected: " + Tokenizer.debugShow(tokens, cnt));
