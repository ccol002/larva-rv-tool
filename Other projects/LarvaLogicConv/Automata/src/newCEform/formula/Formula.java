...
//		
	...
	{		
		...
		if (loc.powerSet.in.contains(i))
		{
			...
			////////////////////////////////////////////////////////////////////////////////////
			if (loc.powerSet.wait2.contains(i))
			{
				if (loc.powerSet.gteq2.contains(i))
				{
			...
			{
				if (loc.powerSet.less2.contains(i))
				{
					CounterInv ci = new CounterInv();
					ci.c = trace.get(i).counter;
					ci.operator = new Operator(Operator.Op.ls);
	...
	public boolean canSeep(Location loc, int i)
	{
		...
		return ((loc.powerSet.in.contains(i-1) && !loc.powerSet.wait.contains(i-1)
					&& !loc.powerSet.wait2.contains(i-1))
...
//			conj.conjunction.add(new newPEA.BoolValue(true));
		...
		invariant.expression = conj;
		 ...
		 for(int i = 0; i < trace.size(); i++)
		 {
					 ...
					 && !trace.get(i).cntupperbound && trace.get(i).cntbounded)
			 {
				 ...
				 newPEA.BoolValue bv3 = new newPEA.BoolValue();
				 bv3.value = succloc.powerSet.wait2.contains(i);
				 BiImplication bi3 = new BiImplication();
				 bi3.lhs = bv3;
				 
				 newPEA.BoolValue bv4 = new newPEA.BoolValue();
				 bv4.value = loc.powerSet.wait2.contains(i);
				...
				BiImplication bi4 = new BiImplication();
				newPEA.BoolValue bv6 = new newPEA.BoolValue();
				bv6.value = succloc.powerSet.gteq2.contains(i);
				bi4.lhs = bv6;
				...
				{
					newPEA.BoolValue bv7 = new newPEA.BoolValue();
					bv7.value = loc.powerSet.gteq2.contains(i)
						&& succloc.powerSet.wait2.contains(i);
					bool4.lhs = bv7;					
				}
				...
				newPEA.BoolValue bv8 = new newPEA.BoolValue();
				bv8.value = !succloc.powerSet.less2.contains(i);
				
				conj.conjunction.add(bv8);
			 }
			 else if (succloc.powerSet.in.contains(i) && trace.get(i).cntupperbound
					 && !canSeep(succloc, i))
			 {
				 BiImplication bi2 = new BiImplication();
				 ...
				 Bool bool3 = new Bool();
				 bool3.lhs = new newPEA.BoolValue(!succloc.powerSet.wait2.contains(i));
				 bool3.op = new BoolOp(BoolOp.Op.and);
				 bool3.rhs = new newPEA.BoolValue(!succloc.powerSet.gteq2.contains(i));
				 conj.conjunction.add(bool3);
				 /////////////////////////////////////
				 
				 BiImplication bi3 = new BiImplication();
				 bi3.lhs = new newPEA.BoolValue(succloc.powerSet.less2.contains(i));
				 ...
				 {
					 bi3.rhs = new newPEA.BoolValue(loc.powerSet.less2.contains(i));
				 }
				 conj.conjunction.add(bi3);
			 }
			 else
			 {
				 ...
				 conj.conjunction.add(new newPEA.BoolValue(!succloc.powerSet.wait2.contains(i)));
				 conj.conjunction.add(new newPEA.BoolValue(!succloc.powerSet.gteq2.contains(i)));
				 conj.conjunction.add(new newPEA.BoolValue(!succloc.powerSet.less2.contains(i)));
			 }
	...
	public ArrayList<Integer> cntlowerbound()
	{
		ArrayList<Integer> lb = new ArrayList<Integer>();
		for (int i = 0; i < trace.size(); i++)
	...
	public Invariant init(Location loc)
	{
		...
		if (loc.powerSet.wait.equals(intersection(loc.powerSet.in, lowerbound()))
				...
				&&
				loc.powerSet.wait2.equals(intersection(loc.powerSet.in, cntlowerbound()))
				&& (trace.get(0).cntop.op.equals(Operator.Op.ls) && loc.powerSet.less2.equals(temp) 
						|| (!trace.get(0).cntop.op.equals(Operator.Op.ls) && loc.powerSet.less2.size()==0))
				&& (loc.powerSet.gteq2.equals(tempGteq)))
		{
			Conjunction conj = new Conjunction();
	...
	public ArrayList<PowerSet> generate()
	{
		...
		for (ArrayList<Integer> list : ins)
		{
			...
			/////////////////////////////////////////////////////////////////////////
			ArrayList<Integer> wait2 = new ArrayList<Integer>();
			ArrayList<Integer> gteq2 = new ArrayList<Integer>();
			for (Integer i : list)
				if (trace.get(i).cntbounded && !trace.get(i).cntupperbound)
				{
					wait2.add(i);
			...
			ArrayList<ArrayList<Integer>> wait2s = power(wait2);
			ArrayList<ArrayList<Integer>> gteq2s = power(gteq2);
			ArrayList<ArrayList<Integer>> less2s = power(less2);
			
			for (ArrayList<Integer> list2 : waits)
				for (ArrayList<Integer> list3 : gteqs)
					for (ArrayList<Integer> list4 : lesses)
						for (ArrayList<Integer> list5 : wait2s)
							for (ArrayList<Integer> list6 : gteq2s)
								for (ArrayList<Integer> list7 : less2s)
					{
	...
	public Invariant getClockInvariant(Location loc)
	{
		...
		for (int i : loc.powerSet.in)
			if (loc.powerSet.wait.contains(i)// || loc.powerSet.wait2.contains(i)
...
//				sb.append("\r\n" + l.eventName + "() = {" + l.events + "}");
		...
		for (Location l : locs)
			if (l.reached && l.powerSet.in.contains(trace.size()-1)
					 ...
					 && !l.powerSet.wait2.contains(trace.size()-1)
					 && !l.powerSet.less2.contains(trace.size()-1))
				sb.append("\r\n " + l.powerSet.toLarva());
		...
		if (l.reached)
		{
			if (l.initial != null)
			{
				...
				conj.conjunction.add(l.getCondition(null,null));
				conj.simplify();
