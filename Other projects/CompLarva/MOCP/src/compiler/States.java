...
public class States extends Compiler{
	...
	public void parseStates(ParsingString ps,State.Type type, ArrayList<State> states) throws ParseException {
		...
		while (cnt < tokens.size())
		{
			...
			//detecting nested state
			if (cnt < tokens.size() && tokens.get(cnt).is("="))
			{
				cnt++;
				if (cnt < tokens.size() && tokens.get(cnt).isIdentifier())
				{
					s.nested = (tokens.get(cnt++));
					State prime = new State();
					prime.id = -s.id;
					prime.type = State.Type.NORMAL;
					prime.name = new Token("_" + s.name.text);
					
					if (!all.containsKey(prime.name))
					{
						all.put(prime.name.text, prime);
						normal.add(prime);
					}
				}	
				else 
