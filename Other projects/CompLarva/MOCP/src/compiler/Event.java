...
//				sb.append("*");
		...
		for (Trigger t : events.values())
			...
			{
				//modified 4-1-11
				EventCollection ec = (EventCollection)t;
				if (ec.events.contains(t))
				{
					List<Token> list = ec.filter;
					if (list.size()>0)
...
//			sb.append("\r\n_cls_inst."+v.name+"="+v.name+";");
			...
			//modified event variables to act as hashmaps 
					
			for (Variable v : variables.values())
				if (v.type != null)
					sb.append("\r\n_cls_inst."+v.name+".put(_cls_"+Global.name + "0.sid," + v.name + ");");
