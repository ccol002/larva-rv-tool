...
//				}
	...
	public void createFileForEach()throws ParseException
	{
		...
		//COMPENSATION MODIFICATION
		//adding channels for nested automata
		for (Property p : logics.values())
		{
				cl.append("\r\npublic static Channel c_" + p.name + "_start = new Channel();");
				cl.append("\r\npublic static Channel c_" + p.name + "_done = new Channel();");
		}
		...
		for (Variable v: events.variables.values())
			if (v.type != null)
				cl.append("\r\npublic static HashMap<Integer," + v.type + "> " + v.name+ "= new HashMap<Integer," + v.type + ">();");
